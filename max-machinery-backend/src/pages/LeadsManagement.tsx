...
const LeadsManagement: React.FC<LeadsManagementProps> = ({ currentTab = TABS.ALL }) => {
  ...
  const {
    ...
    () => {
      ...
      // For now, we'll just fetch data and do client-side filtering
      switch (activeTab) {
        case TABS.PRIORITY:
        ...
        default:
          return leadsApi.getAll(page, pageSize);
      }
    },
    {
  ...
  const filteredLeads = React.useMemo(() => {
    ...
    return leadsData.data.filter(lead => {
      // Apply status filter
      if (filters.status !== 'all' && lead.status?.toLowerCase() !== filters.status.toLowerCase()) {
        return false;
      }
  ...
  }, [leadsData?.data, filters.status, filters.industry, searchTerm]);
